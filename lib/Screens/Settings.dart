...
class _Settings extends State<Settings> with WidgetsBindingObserver {
  ...
  bool hasChangedContent() {
    if (_firstName == _firstNameController.text &&
  ...
  _upLoadImage(File image) async {
    ...
    FormData formData = FormData.fromMap({
      "file": await MultipartFile.fromFile(imageCompress.absolute.path,
          filename: path.basename(imageCompress.path))
    });
    try {
...
//      GlobalData().userData.profile = response.data['path'];
                        ...
                        children: <Widget>[
                          ...
                          Padding(
                            ...
                            child: ProfileTextEditor(
                                ...
                                _phoneController,
                                hint:
                                    'Number will be visible for others to contact'),
                          ),
                          Padding(
                            padding: EdgeInsets.only(top: 40),
                            child: ProfileTextEditor(
  ...
  storeValue(userJson) async {
    ...
    try {
      print('request');
      await http
          .post('${Config.baseURl}/profile/update',
              headers: {
  ...
  _showQR() {
    SaveQR qr = SaveQR(
        '${Config.baseURl}/profile/get?username=${widget.globalData.userData.userName}');
    showDialog(
        context: context,
